...
class BlogCommentBloc extends Object implements Bloc {
  final _commentSubject = PublishSubject<BlogCommentResponse>();
  ...
  Stream<BlogCommentResponse> get commentList => _commentSubject.stream;

  Future<dynamic> getBlogCommentList(String authToken, String blogId) async {
    try {
      BlogCommentResponse blogCommentResponse =
          await _apiHelper.getBlogCommentList(authToken, blogId);

      if (blogCommentResponse.statusCode == 200) {
        _commentSubject.sink.add(blogCommentResponse);
      } else {
        _commentSubject.sink.addError(blogCommentResponse.data.toString());
      }
    } catch (error) {
  ...
  Future<dynamic> submitFeedComment(String authToken, String body) async {
    try {
      PostServiceCommentResponse addCommentModel =
          await _apiHelper.addBlogComment(authToken, body);
  ...
  Future<bool> deleteBlog(String authToken, String commentId) async {
    try {
      DeleteCommentModel deleteCommentModel =
          await _apiHelper.deleteBlogComment(authToken, commentId);
