...
import 'package:solomas/blocs/public_feeds/feed_like_list_bloc.dart';
...
class FeedLikeActivity extends StatefulWidget {
  final String publicFeedId;

  FeedLikeActivity(this.publicFeedId);
...
class _LikesState extends State<FeedLikeActivity> {
  CommonHelper? _commonHelper;

  FeedLikeListBloc? _feedLikeListBloc;

  List<PublicLikeList>? _aList;

  List<PublicLikeList> _searchList = [];
  ...
  void initState() {
    ...
    _feedLikeListBloc = FeedLikeListBloc();
    ...
    _commonHelper?.isInternetAvailable().then((available) {
      if (available) {
        PrefHelper.getAuthToken().then((token) {
          ...
          _feedLikeListBloc?.likeFeed(token.toString(), widget.publicFeedId);
        });
      } else {
  ...
  Widget build(BuildContext context) {
    ...
    return SoloScaffold(
      ...
      body: StreamBuilder(
          stream: _feedLikeListBloc?.likeFeedList,
          builder: (context, AsyncSnapshot<FeedLikeListModel> snapshot) {
            if (snapshot.hasData) {
              if (_aList == null) {
                _aList = snapshot.data?.data?.publicLikeList;
